...
To determine if a flipping occurred, the two checks are preformed. First, check if the area of newly created face is sufficient. Meaning, investigate the angle between edges, if it is bigger by some threshold, edges are almost colinear. If a triangle is defined as $T = (\mathbf{p}, \mathbf{q}, \mathbf{r})$ then:
\begin{align}
...
\end{align}
where $\mathbf{u}$ and $\mathbf{v}$ are unit vectors and $\mathbf{\bar{v}}$ is the new optimal position. Variable $t$ caries the notion of angle between two vectors, if $t$ is bigger than $0.999$ the flipping was introduced and two new edges are almost colinear.
\newpage
...
If $c$ is smaller than $0.2$ it means that the new face is very close to flip, meaning it is close to be perpendicular to the neighbor face, which in many cases introduces inconsistencies. The removal is rejected if it occurs. To correctly investigate flipping, the whole silhouette of neighbors to a given new vertex position has to be checked. The test check has to pass for all of them to accept a decimation.
...
Table \ref{tab:color_simplification} shows the results of using color and geometry metric error. It can be easily noticed that the simplification follows the color pattern. In some cases its a desired features, however, the gradient can introduce noise.
