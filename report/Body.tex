...
There is a few options how we can solve the problem of borders. In my implementation I used a version with adding the quadric error of a plane perpendicular to a given border edge where the perpendicular plane defines a boundary constraint. The other option is to do not touch border vertices. However, this option is problematic if we want to achieve a certain level of simplification. The number of border vertices is by average 15\% of the mesh. It means that this 15\% has to be transferd from the complex shapes which we would like to preserve to the simplification pool. Therefore, we sacrifice the quality of the mesh for preserving edges.
...
The above algorithm is the main core of my implemenation. Every thread will perform the simplification using quadric error metrics, based on Garland's work. Next, I would like to give a brief introduction to extended version of the algoritm which includes color and normals.
...
Table \ref{tab:color_simplification} shows the results of using color and geometry metric error. We can easily notice that the simplifcation follows the color pattern. In some cases its a desired features, however, we have to be carefule about the noise of the gradient.
...
To avoid synchronization overhead I tried deep copies of a sub-meshes passed to processing threads. The problem with this solution is merging sub-meshes into the master mesh after simplification. To preserve local geometry on the edges the process of decemination need not to change or remove them. It gives a problem with convergence since we cannot easily distinguish artificiality created edges due to clustering between original edges which we would like to simplify. Moreover, merging meshes takes time so at the end the algorithm does not gain performance speedup.
...
Figure \ref{fig:my_3} shows approximation of my version of the algorithm. The complex shapes are preserved much better than in all previous versions. However, there is still room for even more aggressive decemination on walls. To achieve it, we need to use more attributes in our joint optimization. 
